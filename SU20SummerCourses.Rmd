...
```{r}
FMNew <- FM_Orig %>% filter(Master_Adname %in% wrongAds)
```
...
data2 <- FMVM %>% 
 ...
 reactable(data2, searchable = TRUE, filterable = TRUE, groupBy = c("Term","Product", "Audience"), columns = list(
...
```{r fig.height=1.5, fig.width=8}
data3 <- FMVM %>% 
  filter( Code_Medium == "DISP" & Code_Vendor == "FB" & Term %in% desiredRecruiting & Impressions > 0 & Date > "2020-06-01" ) %>% 
  group_by(Term, Product, Audience,Date) %>% 
  summarise(
    Impressions = sum(Impressions),
    Cost = sum(Cost),
 ...
 reactable(data3, searchable = TRUE, filterable = TRUE, groupBy = c("Term","Product", "Audience"), columns = list(
...
PGCTR <- data3%>%  
  ...
  a <-  ggarrange(PGImp, PGCTR, PGUPV, PGBR, ncol=4, nrow=1, common.legend = TRUE, legend="right")
  annotate_figure(a, text_grob(paste0(data3$Term, "_ ", data1$Product, "_ ", data3$Audience, " "), color = "darkblue", face = "plain", size = 18, just = "right"))
